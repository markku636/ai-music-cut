...
5. **剪音樂：節拍格線與貼齊**：開檔就從波形能量自動抓 BPM（自相關），時間軸畫出拍線與小節線，選取自動吸附到拍點——剪在拍子上，接起來才不會破。傳輸列可切換顯示 / 貼齊。
6. **去人聲 / 分軌**：一鍵把人聲與伴奏（或 4 軌）分開，伴奏軌就是去人聲版本，可以接著剪。
7. **ASR 驗收（人機協作的最後一關）**：輸出後把成品送回 ttls 重新轉寫，逐字比對「EDL 說該留的字」，標出漏字、該剪沒剪、可疑接縫；點一下就能聽那個位置或直接跳去修。
8. **AI 助手（Claude Code 式工具迴圈）**：本機 `claude` CLI 透過 App 內建的 MCP server 直接操作剪輯決策——「把 10 分鐘後的『就是』都剪掉，但句首的留著」。
