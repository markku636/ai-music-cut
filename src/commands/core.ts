...
export const CORE_COMMANDS: Command[] = [
  ...
  { id: "record.new", title: "錄音…", group: "file", section: "檔案", icon: Mic, surfaces: ["menu", "palette", "simple"], simple: true, simpleLabel: "錄音", simpleHint: "用麥克風先錄一段，錄完直接剪", keywords: ["record", "mic", "microphone"], enabled: () => OK, run: () => openDialog("record", { mode: "new" }) },
  {
    id: "record.retake",
    title: "重錄這句（選取範圍）",
    group: "edit",
    ...
    simple: true,
    simpleLabel: "重錄這句",
    simpleHint: "選一段講壞的，聽一次再錄，新錄的會自動塞回去",
    simpleOrder: 7,
    keywords: ["retake", "punch in", "redub", "re-record"],
    enabled: needsSelection,
    run: () => openDialog("record", { mode: "retake", range: useTimeline.getState().selection }),
  },
