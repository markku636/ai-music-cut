...
30. **輸出後的響度驗收（而且會說「為什麼沒打到」）**：輸出完成後不只印一個數字，而是判定有沒有達標（±0.5 LU 內達標、±1.5 內警告、再多就是未達標），真實峰值也一起看，並且在**沒打到目標時說明原因**。
   ...
   **還會告訴你成品有沒有被動態壓縮過。** 輸出送給 ffmpeg 的是 `linear=true`（只套一個增益、不改動態），但那只是「請求」—— 需要的增益會讓峰值超過上限時，ffmpeg 會**自己退回 dynamic**，也就是動態壓縮，成品的音量起伏被壓平。它只在 loudnorm 的 JSON 裡講一次 `normalization_type`，不解析就永遠不知道自己交出去的是壓過的檔案。實測這個 App 自己的樣本：**一直都是 `dynamic`**。
