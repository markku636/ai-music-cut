...
21. **AI 助手（Claude Code 式工具迴圈）**：本機 `claude` CLI 透過 App 內建的 MCP server 直接操作剪輯決策——「把 10 分鐘後的『就是』都剪掉，但句首的留著」。44 支工具涵蓋決策、接縫與修剪（`list_seams` / `blade_at` / `trim_seam` / `insert_pause` / `set_selection` / `lift_selection` / `add_effect`）、逐字稿（`find_text` / `cut_text`）、標記與章節、配樂與閃避、多麥同步、修聲（`get_cleanup` / `set_cleanup`）、精華片段（`list_highlights` / `add_highlight`）、節目筆記（`write_show_notes` / `get_show_notes`）、講者（`list_speakers` / `assign_speaker` / `rename_speaker` / `cut_fillers_by_speaker`）、字幕與分割（`export_captions` / `list_split_parts`）、響度輪廓（`get_loudness_profile` —— 助手聽不到聲音，這支讓它看得到），所以也接得住「把來賓的口頭禪都剪掉、主持人的留著」、「產一份帶講者名字的 SRT」或「在 12:30 切一刀，接縫往左推 200 毫秒」或「看看底噪要不要修，順便把整集的口頭禪剪掉」。
