...
7. **Markers, timeline index, chapter export**: `M` drops a marker, `Shift+M` a chapter, `Alt+M` a to-do, `Alt+[` `Alt+]` jump between them (the waveform right-click menu and the Index tab's "add at playhead" do the same thing — no shortcut memorising required). Pins are drawn on the time ruler, draggable, right-clickable to change type. The **Index** tab on the right flattens the whole timeline into one searchable, filterable list (chapters / markers / to-dos / seams / effects); click to jump, edit chapter titles inline. **Chapters are written into the output file**: ID3 CHAP for mp3, QuickTime chapters for m4a (Apple Podcasts and Spotify read them), with times converted automatically from *source* to *after the cut*. To-dos can be ticked off; the open count shows on the tab.
...
16. **Skimming**: turn it on with `Shift+S` and moving the mouse over the waveform plays the audio under the cursor. Far faster than dragging the playhead to find "where was that line". Off by default — audio that plays because the mouse passed over something needs consent. It gets out of the way during playback so two sounds never stack.
...
38. **Simple / pro dual mode (for absolute beginners)**: a fresh install opens in **simple mode**: the start screen asks "what would you like to do?" (edit a podcast / remove noise / edit music…), and once a file is open the right-hand side is just a handful of plain-language buttons — auto-remove fillers, remove noise, even out the volume, cut the selected part, keep only the selected part, add intro / outro music, export mp3 — with "Undo" always at the top, and every action that touches the audio finishes with a one-line toast plus an undo button (beginners don't know Ctrl+Z). Right-clicking a range shows a flat seven-item menu with no submenus; the transport is just play, time, a zoom slider and loop. The toggle in the top-right switches to **pro mode** with the full UI (side panel, precision trim, shortcuts, command palette); the file, edits and selection all carry over.
   **Both are the same commands and the same waveform** (`SimpleShell` / `ProShell` both mount `Workspace` + `TranscriptArea`); simple mode removes things, it is not a second app. Existing users keep pro mode after upgrading — nobody gets dropped into simple.
   > The beginner acceptance run drives the real UI over CDP: open → remove noise → drag a range, right-click, cut (undo via the toast) → export mp3, using only panel buttons and right-click, zero shortcuts, eight clicks.
