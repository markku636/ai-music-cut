...
import { mergeOutPath, mergeTotalMs, moveItem, normalizeGains, type MergeItem, type MergeJoin } from "../analysis/mergePlan";
...
export default function MergeDialog({ onClose }: { onClose: () => void }) {
  ...
  const addFiles = async () => {
    ...
    for (const p of list) {
      ...
      if (!durationMs) durationMs = await api.mediaProbe(p).then((x) => x.duration_ms).catch(() => 0);
      const local = known ? useTranscript.getState().local[known.id] : null;
      const lufs = local ? (loudnessProfile(local)?.episodeLufs ?? null) : null;
      added.push({ id: `${p}#${Date.now()}`, path: p, name: p.split(/[\\/]/).pop() ?? p, durationMs, lufs, gainDb: 0 });
    }
  ...
  const addFromProject = async () => {
    ...
    for (const m of media) {
      ...
      added.push({ id: m.id, path: m.path, name: m.name, durationMs: m.probe?.duration_ms ?? 0, lufs: local ? (loudnessProfile(local)?.episodeLufs ?? null) : null, gainDb: 0 });
    }
    setItems((cur) => [...cur, ...added]);
  };

  const gains = level ? normalizeGains(items) : items.map((i) => i.gainDb);
  const totalMs = mergeTotalMs(items, join, joinMs);
  ...
  const run = async () => {
    ...
    try {
      const r = await api.mergeFiles({ inputs: items.map((i, k) => ({ path: i.path, gain_db: gains[k] })), join, join_ms: joinMs, channels: 1, out_path: outPath });
