...
export default function TransportBar({ durationMs }: { durationMs: number }) {
  const t = useT();
  ...
  return (
    <div className="h-10 shrink-0 flex items-center gap-1 px-2 border-b border-fg/10 bg-panel">
      ...
      <button
        ...
        title={t("播放速率")}
        className="h-7 px-2 rounded-sm text-xs mono text-fg/70 hover:bg-fg/5"
      >
        {rate}×
      </button>
      <span className="mono text-xs text-fg/70 ml-2 tabular-nums">
        {formatMs(currentMs)} <span className="text-fg/30">/ {formatMs(durationMs)}</span>
      </span>
      <div className="ml-auto flex items-center gap-1">
        <IconButton icon={Scissors} label={t("跳過剪除區段")} active={skipEnabled} onClick={toggleSkip} />
